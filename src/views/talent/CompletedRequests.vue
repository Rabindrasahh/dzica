<template>
	<div class="card">
		<div class="card-header bg-gradient-main">
			<h4 class="text-white">Completed Requests</h4>
		</div>
		<div class="card-body">
...
export default {
	...
	methods: {
		async init(page = 1) {
			this.isLoading = true;
			try {
				await this.$store.dispatch("talent/getCompletedRequests", {
					skip: (page - 1) * this.dataInEachPage,
					limit: this.dataInEachPage,
				});
				this.totalItems =
					this.$store.getters["talent/completedRequests"].totalItems;
			} catch (err) {
				this.$swal({
					icon: "error",
					text: err.message,
	...
	computed: {
		requests() {
			return this.$store.getters["talent/completedRequests"].requests;
		},
	},
	created() {
		this.init();
